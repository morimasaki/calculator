...
(function() {
    ...
    enchant.Core = enchant.Class.create(enchant.EventTarget, {
         ...
         * @name enchant.Core
         * @class
         * A class for controlling the core窶冱 main loop and scenes.
         *
...
enchant.Node = enchant.Class.create(enchant.EventTarget, {
    ...
    initialize: function() {
        ...
        // Node縺檎函謌舌＆繧後ｋ髫帙↓, tl 繝励Ο繝代ユ繧｣縺ｫ Timeline 繧ｪ繝悶ず繧ｧ繧ｯ繝医ｒ霑ｽ蜉縺励※縺�ｋ.
...
enchant.Action = enchant.Class.create(enchant.ActionEventTarget, {
     ...
     * @name enchant.Action
     * @class
ﾂﾂﾂﾂﾂ* Actions are units that make up the timeline.
ﾂﾂﾂﾂ * It is a unit used to specify the action you want to perform.
ﾂﾂﾂﾂ * 
ﾂﾂﾂﾂ * Actions that have been added to the timeline are performed in sequential order.
ﾂﾂﾂﾂ * The transition from one action to the next occurs automatically 
ﾂﾂﾂﾂ * after the number of frames specified by the time parameter have elapsed.
ﾂﾂﾂﾂ *
ﾂﾂﾂﾂ * An actionstart event is fired when the action has started.
ﾂﾂﾂﾂ * An actionend event is fired when the action has stopped.
ﾂﾂﾂﾂ * For each frame that elapses, an actiontick event is fired.
ﾂﾂﾂﾂ * 
ﾂﾂﾂﾂ * You can specify a listener for these events to perform specific events when they occur.
     *
     * @param {Object} param
